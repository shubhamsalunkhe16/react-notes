...
`forwardRef` lets you **pass a `ref` through a component to a child DOM node** — useful when you want the parent to directly access a DOM element inside a custom component.

#### 🧠 Why?

By default, a `ref` on a custom component points to the **component instance**, **not** a DOM element. `forwardRef` lets you expose the **inner DOM node** to the parent.
...
#### 🧑‍💻 Real Example – Focus a Custom Input
...
- `inputRef` points directly to `<input>` inside `MyInput`
- Useful for focusing, measuring, or scrolling
